...
def process_brenda_data(brenda_data):
    ...
    for ec_number, data in brenda_data.items():
       ...
       # Add pathway categories - transform from scoring format to expected format
        ...
        corrosion_relevant_metals = [metal for metal in cs.metal_terms if metal in cs.metal_mapping]

        processed_data[ec_number]['metal_mapping_from_brenda'] = [
        ...
        pathway_score, pathway_category_scores = cs.calculate_pathway_score(
            processed_data.get('pathways', []), enzyme_names, enzyme_class
        )
        ...
        corrosion_relevance_score, corrosion_relevance = cs.calculate_corrosion_relevance_score(
            processed_data.get('overall_metal_score', 0), 
            processed_data.get('overall_corrosion_score', 0), 
            processed_data.get('overall_organic_process_score', 0),
            processed_data.get('overall_keyword_score', 0),
            processed_data.get('overall_functional_score', 0)
        )
        ...
        corrosion_score += len(processed_data[ec_number]['metal_mapping_from_brenda']) * 1.5
...
def create_metabolism_database(sample_size=None):
    ...
    for ec_number, names in ec_to_names.items():
        ...
        record['metal_mapping_from_brenda'] = [] # from the global terms
        ...
        if brenda_en and ec_number in brenda_en:
            ...
            record['metal_mapping_from_brenda'] = brenda_en[ec_number].get('metal_mapping_from_brenda', [])
